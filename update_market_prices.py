...
def normalize_quote(key,label,price,previous_close,source_ts,source_name,now=None):
    ...
    price=float(price)
    previous=float(previous_close) if previous_close not in (None,0) else None
...
def unavailable_or_cached(key,label,cached,error):
    if cached and cached.get('price') is not None:
      row=dict(cached)
      row.update({'key':key,'label':label,'status':'Last available','error':str(error)[:180]})
      return row
    return {
      'key':key,'label':label,'price':None,'change':None,'changePct':None,
      'sourceTimestamp':None,'sourceName':None,'status':'Unavailable','error':str(error)[:180]
    }

def fetch_text(url):
    req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 GlobalEventsDashboard/4.0','Accept':'application/json,*/*'})
...
def fetch_yahoo(symbol,key,label,now):
    ...
    if price is None or ts is None:
      raise RuntimeError(f'incomplete quote for {symbol}')
...
def main():
    ...
    if OUTPUT.exists():
      try:
        cached={r.get('key'):r for r in json.loads(OUTPUT.read_text()).get('instruments',[])}
      except Exception:
        cached={}
    rows=[]
    health={}
    for key,(label,symbols) in INSTRUMENTS.items():
      ...
      for symbol in symbols:
        try:
          row=fetch_yahoo(symbol,key,label,now)
          health[key]={'status':'ok','symbol':symbol}
          break
        except Exception as exc:
          errors.append(str(exc))
      if row is None:
        row=unavailable_or_cached(key,label,cached.get(key),'; '.join(errors))
        health[key]={'status':'error','error':row.get('error')}
      rows.append(row)
    payload={
