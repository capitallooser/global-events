...
NSEIX_HOME='https://www.nseix.com/'

def parse_nseix_gift(text,now=None):
    now=now or datetime.now(timezone.utc)
    compact=' '.join(text.split())
    m=re.search(r'(\d{1,3}(?:,\d{3})+(?:\.\d+)?)\s+([+-]?\d+(?:\.\d+)?)\s*\(([+-]?\d+(?:\.\d+)?)%\).*?Current Day.*?Date\s*:\s*(\d{1,2}-[A-Za-z]{3}-\d{4})',compact,re.I)
    if not m: raise RuntimeError('unable to parse GIFT Nifty from NSE IX')
    price=float(m.group(1).replace(',','')); change=float(m.group(2)); pct=float(m.group(3))
    d=datetime.strptime(m.group(4),'%d-%b-%Y').replace(hour=15,minute=30,tzinfo=IST).astimezone(timezone.utc)
    age=max(0,(now-d).total_seconds())
    status='Live' if age<=45*60 else ('Delayed' if age<=24*3600 else 'Last available')
    return {'key':'gift_nifty','label':'GIFT Nifty','price':round(price,4),'change':round(change,4),'changePct':round(pct,3),'sourceTimestamp':d.replace(microsecond=0).isoformat().replace('+00:00','Z'),'sourceName':'NSE IX','status':status}
...
def main():
    ...
    for key,(label,symbols) in INSTRUMENTS.items():
      ...
      if row is None and key=='gift_nifty':
        try:
          raw=fetch_text(NSEIX_HOME); plain=re.sub(r'<[^>]+>',' ',raw); row=parse_nseix_gift(plain,now); health[key]={'status':'ok','symbol':'NSE IX homepage'}
        except Exception as exc: errors.append(f'NSE IX: {exc}')
      if row is None:
