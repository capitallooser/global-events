...
def fetch_text(url,timeout=35):
    req=urllib.request.Request(url,headers={'User-Agent':'Mozilla/5.0 GlobalEventsMarketIntelligence/3.0','Accept':'*/*'})
...
def reaction_return(prices,event_date,benchmark_market,event_origin,horizon):
    dates=sorted(prices);event=event_date.isoformat()
...
def build_analysis(event_dates_by_key,benchmark_prices,benchmark_sources,now):
    ...
    for key,meta in EVENT_TYPES.items():
      ...
      for bkey,cfg in BENCHMARKS.items():
        ...
        one_stats=summarize([reaction_return(prices,d,cfg['market'],meta['origin'],1) for d in dates]);three_stats=summarize([reaction_return(prices,d,cfg['market'],meta['origin'],3) for d in dates])
      ...
      event_types[key]={'label':meta['label'],'historySource':meta['historySource'],'historicalEvents':len(dates),'impactScore':round(score,2) if score is not None else None,'impactLevel':impact_level(score),'markets':markets}
    return{'updatedAt':now.replace(microsecond=0).isoformat().replace('+00:00','Z'),'methodology':{'minimumDirectionalSample':8,'oneDayDefinition':'Close-to-close first reaction session','threeDayDefinition':'Close-to-close through third reaction session','indiaForUSEvents':'US events are measured from the Indian event-day close to following Indian trading sessions because major U.S. releases occur after the Indian cash-market close.','disclaimer':'Historical frequency and movement are descriptive statistics, not a forecast or guarantee.'},'benchmarks':{k:{'label':v['label'],'priceSource':benchmark_sources.get(k),'points':len(benchmark_prices.get(k,{}))} for k,v in BENCHMARKS.items()},'eventTypes':event_types}

def main():
    now=datetime.now(timezone.utc);start_year=max(2018,now.year-8);by={k:[] for k in EVENT_TYPES};status={'history':{},'prices':{}}
    ...
    result=build_analysis(by,prices,sources,now);result['status']=status;tmp=OUTPUT.with_suffix('.tmp');tmp.write_text(json.dumps(result,indent=2,ensure_ascii=False)+'\n');tmp.replace(OUTPUT)
