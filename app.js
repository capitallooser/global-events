...
function renderEventList(target,events,limit=80){const rows=events.slice(0,limit);$(target).innerHTML=rows.length?rows.map(eventCard).join(''):'<div class="empty">No matching upcoming events.</div>';bindEventButtons();}
...
function openDay(iso,events){const rows=events.filter(e=>e.start.slice(0,10)===iso).sort((a,b)=>a.start.localeCompare(b.start));$('detailContent').innerHTML=`<p class="eyebrow">${esc(iso)}</p><h2>${rows.length} event${rows.length===1?'':'s'}</h2>${rows.map(eventCard).join('')||'<p>No events.</p>'}`;$('detailDialog').showModal();bindEventButtons();}
...
function renderDataViews(preserveScroll=false){const y=window.scrollY;renderOverview();renderRegion();renderRanked();renderCalendar();renderAlertState();renderCount();if(preserveScroll)requestAnimationFrame(()=>window.scrollTo({top:y,left:0,behavior:'auto'}));}
function applyFilters(){renderOverview();renderRegion();renderRanked();renderCount();}
function switchTab(tab){currentTab=tab;document.querySelectorAll('.tabs button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));document.querySelectorAll('.tab-panel').forEach(p=>p.classList.remove('active'));$(`${tab}Panel`)?.classList.add('active');updateFilterVisibility();renderMarketStrip();if(tab==='calendar')renderCalendar();if(['overview','india','us','global','movers'].includes(tab))applyFilters();window.scrollTo({top:0,behavior:'smooth'});}
function markClientRefresh(kind){const stamp=new Date().toLocaleTimeString('en-IN',{timeZone:'Asia/Kolkata',hour:'2-digit',minute:'2-digit',hour12:true});$('liveRefreshState').textContent=`Auto-refresh on · ${kind} ${stamp} IST`;}

async function refreshMarketData(){try{marketPrices=await fetchJSON('./market-prices.json');renderMarketStrip();markClientRefresh('markets refreshed');}catch(err){$('liveRefreshState').textContent='Auto-refresh on · market refresh retrying';console.warn(err);}}
async function refreshDashboardData(){const results=await Promise.allSettled([fetchJSON('./events.json'),fetchJSON('./source-status.json'),fetchJSON('./market-impact.json'),fetchJSON('./surprises.json'),fetchJSON('./alert-state.json')]);if(results[0].status==='fulfilled')all=results[0].value;if(results[1].status==='fulfilled')renderUpdateStatus(results[1].value);if(results[2].status==='fulfilled')marketImpact=results[2].value;if(results[3].status==='fulfilled')surprises=results[3].value.events||results[3].value||{};if(results[4].status==='fulfilled')alertState=results[4].value;renderDataViews(true);markClientRefresh('events refreshed');}

document.querySelectorAll('[data-win]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-win]').forEach(x=>x.classList.remove('active'));b.classList.add('active');filterWindow=b.dataset.win;applyFilters();});
['q','country','category','importance','impact','sort'].forEach(id=>$(id).addEventListener(id==='q'?'input':'change',applyFilters));
...
try{
  ...
  updateFilterVisibility();renderDataViews();renderMarketStrip();markClientRefresh('ready');
  setInterval(refreshMarketData,LIVE_REFRESH_MS.market);
  setInterval(refreshDashboardData,LIVE_REFRESH_MS.data);
